...
{
    public class NotaDeCreditoInterna : DocumentoElectronicoVenta
    {
        public override string NombreTipo { get; set; } = "NOTA DE CRÉDITO INTERNA";
        ...
        public static List<NotaDeCreditoInterna> Convert(List<OperacionDeVenta> operaciones, EstablecimientoComercialExtendido sede, List<Detalle_maestro> tiposDeNotaDeCredito, ModoImpresionCaracteristicasEnum modoImpresionCaracteristicas)
        {
            List<NotaDeCreditoInterna> resultado = new List<NotaDeCreditoInterna>();
            foreach (var item in operaciones)
            {
                resultado.Add(new NotaDeCreditoInterna(item, sede, new EstablecimientoComercialExtendidoConLogo(item.Transaccion().Actor_negocio2.Actor_negocio2), tiposDeNotaDeCredito, null, false, modoImpresionCaracteristicas));
            }
            return resultado;
        }
    }
